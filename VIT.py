...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, patch_size, max_len, embed_dim, layers, channels=3, heads=8, activation=None,
    ...
    def forward(self, inp):
        ...
        ### unfold(dim=滑动的方向, size=滑动窗口大小, step=滑动步长)；此步骤对h和w方向做滑窗
        ### wout = (win-size)/step +1
        out = inp.unfold(2, self.patch_size, self.patch_size).unfold(3, self.patch_size, self.patch_size).contiguous()
        out = out.view(batch_size, channels, -1, self.patch_size, self.patch_size)
        out = out.permute(0, 2, 3, 4, 1)
        # out: (batch_size, seq_len, patch_size, patch_size, channels) | seq_len = (width*height)/(patch_size**2)
        batch_size, seq_len, patch_size, _, channels = out.size()

        out = out.reshape(batch_size, seq_len, -1)
        out = self.patch_to_embed(out) ## out: (batch_size, seq_len, embed_dim)

        position_embed = self.position_embed[:seq_len + 1] ## [max_len, embed_dim]
        position_embed = position_embed.unsqueeze(0).expand(batch_size, seq_len, self.embed_dim)
        out = out + position_embed # out: (batch_size, seq_len, embed_dim)

        ''' transformerde 部件的输出 '''
        out = self.transformer(out)  # out: (batch_size, seq_len, embed_dim)
        ## out[:, i] == [b, embed_dim], i为每个压缩向量点，长度为需求长度,
        return out

if __name__=='__main__':

    input = torch.randn(1, 3, 1, 1024)


    ### patch_size=滑动窗口的大小，以及步长
    ## max_len可以理解为序列的长度，也可以改造为像素值，及分割中的w*h个像素，一个像素的压缩表达，长度为embed_dim
    ...
    ### model的输入可以是与训练网络的输出，VisionTransformer可以作为一个中间部件
    t_out = model(input)
    print(t_out.shape)
